...
@router.get("/process_command", response_class=HTMLResponse)
async def process_command(request: Request):
    return templates.TemplateResponse("process_command.html", {"request": request})


@router.post("/process_command", response_class=HTMLResponse)
async def process_command(request: Request, audioFile: UploadFile = File(...), db: Session = Depends(get_db)):
    try:
        # Temporäre Datei für die Audiodatei erstellen
        with tempfile.NamedTemporaryFile(delete=False, suffix=".wav") as temp_audio_file:
            temp_audio_file.write(await audioFile.read())
        ...
        # Anfrage an OpenAI stellen, um die gewünschte Aktion zu identifizieren
        response = openai.ChatCompletion.create(
            model="gpt-4-turbo",
            messages=[
                {"role": "system", "content": "Du bist ein hilfreicher Assistent."},
