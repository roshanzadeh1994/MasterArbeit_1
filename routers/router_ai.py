...
async def process_signup(userText: str = Form(...), db: Session = Depends(get_db)):
    try:
        ...
        return HTMLResponse(content=f"<h1>Extrahierte Daten:</h1><p>Benutzername: {ai_user_data['benutzername']}</p>"
                                    f"<p>E-Mail: {ai_user_data['e-mail']}</p>"
                                    f"<p>Passwort: {ai_user_data['passwort']}</p>")
